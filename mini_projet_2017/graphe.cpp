...
void Graphe::grapheOrdonnancement(ofstream& fichier_resultat) {
    ...
    if (fichier_resultat)
    {
        ...
        for (int i = 0; i < taches; i++)
        {
            for (int j = 0; j < tabSommets[i].getNombreContraintes(); j++)
            {
                if (tabSommets[i].getNombreContraintes() >= 1)
                {
        ...
        for (int i = 0; i < taches; i++)
        {
            sommet_actuel = tabSommets[i].getNomSommet();
            ...
            for (int j = 0; j < taches; j++)
            {
                if (i != j)
                {
                    sommet_cible = tabSommets[j].getNomSommet();
                    
                    for (int k = 0; k < tabSommets[j].getNombreContraintes(); k++)
                    {
                        if (tabSommets[j].getContrainte(k) == sommet_actuel)
                        {
                            nombre_successeurs++;
                    ...
                    tabSommets[i].setNombreSuccesseurs(nombre_successeurs);
                }
            }
        }
    }
...
void Graphe::ajoutSommetsIncidents(ofstream& fichier_resultat) {
    ...
    int i = 0;
    do {
        nombre_contraintes = tabSommets[i].getNombreContraintes();
        nombre_successeurs = tabSommets[i].getNombreSuccesseurs();
        nom_sommet = tabSommets[i].getNomSommet();
...
void Graphe::initMatrices(ofstream& fichier_resultat) {
    ...
    {
        predecesseur_actuel = tabArcs[i].getPredecesseur();
        sommet_actuel = tabArcs[i].getNomSommet();
        duree_actuelle = tabArcs[i].getDuree();
        
        cout << " * Arc " << (i + 1) << ": " << tabArcs[i].getPredecesseur() << " --[" << tabArcs[i].getDuree() << "]--> " << tabArcs[i].getNomSommet() << endl;
        fichier_resultat << " * Arc " << (i + 1) << ": " << tabArcs[i].getPredecesseur() << " --[" << tabArcs[i].getDuree() << "]--> " << tabArcs[i].getNomSommet() << endl;
...
void Graphe::affichageMatriceAdjacence() const
{
    ...
    for (int i = 0; i < taches; i++)
    {
        if (i == 0)
        {
            cout << "    " << tabSommets[i].getNomSommet() << "  ";
        }
        else
        {
            cout << tabSommets[i].getNomSommet() << "  ";
...
void Graphe::initRangsSommets(ofstream& fichier_resultat)
{
    ...
    for (int i = 0; i < taches; i++)
    {
        sommet_cible = tabSommets[i].getNomSommet();
        ...
        tabSommets[i].setRangSommet(rangs_sommets[sommet_cible]);
    }
}
